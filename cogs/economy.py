...
default_settings = {"PAYDAY_TIME": 180, "PAYDAY_CREDITS": 500,
...
class Economy:
    ...
    async def register(self, ctx):
        """Registers an account at the Hellion bank"""
        ...
        try:
            ...
            await self.bot.say("{} You already have an account at the"
                               " Hellion bank.".format(author.mention))
    ...
    async def _set(self, ctx, user: discord.Member, credits: SetParser):
        ...
        Examples:
            bank set @Diru 26 - Sets 26 credits
            bank set @Diru +2 - Adds 2 credits
            bank set @Diru -6 - Removes 6 credits"""
        author = ctx.message.author
        try:
