...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes NixiaBot's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start Nixia "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets NixiaBot's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets NixiaBot's prefixes for this server
    ...
    async def game(self, ctx, *, game=None):
        """Sets NixiaBot's playing status
    ...
    async def _blacklist_add(self, user: GlobalUser):
        """Adds user to NixiaBot's global blacklist"""
    ...
    async def whitelist(self, ctx):
        ...
        be able to use NixiaBot"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @whitelist.command(name="add")
    async def _whitelist_add(self, user: GlobalUser):
        """Adds user to NixiaBot's global whitelist"""
    ...
    async def _whitelist_remove(self, user: GlobalUser):
        """Removes user from NixiaBot's global whitelist"""
    ...
    async def version(self):
        """Shows NixiaBot's current version"""
